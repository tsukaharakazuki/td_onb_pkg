...
SELECT 
  user_id,
  MAX(CASE WHEN key = '${set.output}${set.name1}' THEN vals END) AS ftr3_${set.output}${set.name1} ,
  MAX(CASE WHEN key = '${set.output}${set.name2}' THEN vals END) AS ftr3_${set.output}${set.name2} ,
  MAX(CASE WHEN key = '${set.output}${set.name3}' THEN vals END) AS ftr3_${set.output}${set.name3} 
FROM 
  t1
GROUP BY
  1